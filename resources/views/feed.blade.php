<x-public-layout>
    ...
    <div x-data="{ 
        ...
        class="max-w-7xl mx-auto w-full mt-6 px-4 flex flex-col lg:flex-row gap-8 relative pb-24 lg:pb-0">
        ...
        <div x-cloak 
             ...
             ">
            <div class="w-full flex justify-center pt-3 pb-1" @click="searchOpen = false">
                ...
                <div class="grid grid-cols-2 gap-3">
                    @foreach($categories as $cat)
                        <a href="{{ route('feed', array_merge(request()->query(), ['category' => $cat->id])) }}" 
                           class="flex flex-col p-3 rounded-xl border transition-all h-full justify-center {{ request('category') == $cat->id ? 'bg-maroon-700 text-white border-maroon-700 shadow-md transform scale-[1.02]' : 'bg-gray-50 border-transparent text-gray-600 hover:bg-white hover:border-gray-200 hover:shadow-sm' }}">
        ...
        <div x-cloak 
            ...
            <div class="p-4 space-y-2">
                ...
                <a href="{{ route('feed', array_merge(request()->except('sort'), ['sort' => null])) }}" 
                   class="flex items-center justify-between p-4 rounded-xl {{ !$sort ? 'bg-maroon-50 text-maroon-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
                    <span><i class='bx bx-time-five mr-2'></i> Newest</span>
                    @if(!$sort) <i class='bx bx-check text-xl'></i> @endif
                </a>
                <a href="{{ route('feed', array_merge(request()->query(), ['sort' => 'popular'])) }}" 
                   class="flex items-center justify-between p-4 rounded-xl {{ $sort === 'popular' ? 'bg-maroon-50 text-maroon-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
                    <span><i class='bx bx-trending-up mr-2'></i> Popular</span>
                    @if($sort === 'popular') <i class='bx bx-check text-xl'></i> @endif
                </a>
                <a href="{{ route('feed', array_merge(request()->query(), ['sort' => 'discussed'])) }}" 
                   class="flex items-center justify-between p-4 rounded-xl {{ $sort === 'discussed' ? 'bg-maroon-50 text-maroon-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
        ...
        <div x-cloak 
            ...
            <div class="p-4 space-y-2">
                ...
                <a href="{{ route('feed', array_merge(request()->except('filter'), ['filter' => null])) }}" 
                   class="flex items-center justify-between p-4 rounded-xl {{ !$filter ? 'bg-maroon-50 text-maroon-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
                    <span>All Questions</span>
                    @if(!$filter) <i class='bx bx-check text-xl'></i> @endif
                </a>
                <a href="{{ route('feed', array_merge(request()->query(), ['filter' => 'solved'])) }}" 
                   class="flex items-center justify-between p-4 rounded-xl {{ $filter === 'solved' ? 'bg-maroon-50 text-maroon-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
                    <span><i class='bx bx-check-circle mr-2 text-green-600'></i> Solved</span>
                    @if($filter === 'solved') <i class='bx bx-check text-xl'></i> @endif
                </a>
                <a href="{{ route('feed', array_merge(request()->query(), ['filter' => 'unsolved'])) }}" 
                   class="flex items-center justify-between p-4 rounded-xl {{ $filter === 'unsolved' ? 'bg-maroon-50 text-maroon-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
                ...
                <a href="{{ route('feed', array_merge(request()->query(), ['filter' => 'no_answers'])) }}" 
                   class="flex items-center justify-between p-4 rounded-xl {{ $filter === 'no_answers' ? 'bg-maroon-50 text-maroon-700 font-bold' : 'text-gray-600 hover:bg-gray-50' }}">
                    ...
                    <div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-6 mb-8 rounded-lg shadow-sm flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-light mt-1 text-gray-600">You must verify your email address before you can post.</p>
                            </div>
                        </div>
                        <form method="POST" action="{{ route('verification.send') }}">
                            ...
                            <button type="submit" class="bg-red-700 text-white px-4 py-2 rounded-lg font-normal hover:bg-red-800 transition shadow-sm flex items-center tracking-wide">
                                <i class='bx bx-mail-send mr-2'></i> Resend Link
                            </button>
                        </form>
                    </div>
                @endif
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="hidden lg:flex items-center gap-3">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" @click.outside="open = false" class="flex items-center space-x-1 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-700 shadow-sm hover:bg-gray-50 transition">
                            <i class='bx bx-sort-alt-2 text-base'></i>
                            <span>{{ request('sort') === 'popular' ? 'Popular' : (request('sort') === 'discussed' ? 'Hot' : 'Newest') }}</span>
                        ...
                        <div x-show="open" 
                             ...
                             class="absolute right-0 mt-2 w-40 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50 origin-top-right">
                            
                            <a href="{{ route('feed', array_merge(request()->except('sort'), ['sort' => null])) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-maroon-700 {{ !request('sort') ? 'font-bold text-maroon-700 bg-maroon-50' : '' }}">
                                Newest
                            </a>
                            <a href="{{ route('feed', array_merge(request()->query(), ['sort' => 'popular'])) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-maroon-700 {{ request('sort') === 'popular' ? 'font-bold text-maroon-700 bg-maroon-50' : '' }}">
                                Popular
                            </a>
                            <a href="{{ route('feed', array_merge(request()->query(), ['sort' => 'discussed'])) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-maroon-700 {{ request('sort') === 'discussed' ? 'font-bold text-maroon-700 bg-maroon-50' : '' }}">
                                Hot (Discussed)
                            </a>
                        </div>
                    </div>
        ...
        <nav class="lg:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-[80] pb-safe">
            <div class="flex justify-between items-center h-16 px-6">
                ...
                <button @click="mobileMenuOpen = !mobileMenuOpen; searchOpen = false; sortOpen = false; filterOpen = false; leaderboardOpen = false" class="flex flex-col items-center justify-center space-y-1 {{ $mobileMenuOpen ?? false ? 'text-maroon-700' : 'text-gray-400 hover:text-gray-600' }}">
                ...
                <button @click="searchOpen = !searchOpen; mobileMenuOpen = false; sortOpen = false; filterOpen = false; leaderboardOpen = false" class="flex flex-col items-center justify-center space-y-1 {{ $searchOpen ?? false ? 'text-maroon-700' : 'text-gray-400 hover:text-gray-600' }}">
