                                ...
                                @if($report->question)
                                    <div class="mb-1">
                                        <a href="{{ route('question.show', $report->question->id) }}" target="_blank" class="text-maroon-700 font-bold hover:underline text-sm flex items-center">
                                            ...
                                            <i class='bx bx-link-external ml-2 text-xs opacity-50'></i>
                                        </a>
                                    </div>
                                    <p class="text-sm text-gray-500 font-light leading-relaxed line-clamp-2">
                                        {{ Str::limit($report->question->content, 120) }}
                                    </p>
                                @else
                                    <span class="text-gray-300 italic text-sm flex items-center">
                                        <i class='bx bx-trash mr-1'></i> Content deleted
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-5 align-top text-right">
                                <div class="flex justify-end space-x-3">
                                    <form action="{{ route('admin.dismiss_report', $report->id) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="flex items-center px-3 py-1.5 text-xs text-gray-500 hover:text-gray-800 border border-gray-300 hover:border-gray-500 rounded transition bg-transparent" title="Dismiss">
                                            <i class='bx bx-x mr-1 text-base'></i> Dismiss
                                        </button>
                                    </form>
                                    @if($report->question)
                                    <form action="{{ route('admin.delete_question', $report->question->id) }}" method="POST" onsubmit="return confirm('Delete this question?');" class="inline">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="flex items-center px-3 py-1.5 text-xs text-red-600 hover:text-white border border-red-200 hover:bg-red-600 hover:border-red-600 rounded transition bg-transparent" title="Delete">
                                            <i class='bx bx-trash mr-1 text-base'></i> Delete
                                        </button>
                                    </form>
                                    @endif
                                </div>
            ...
            @else
                <div class="py-24 text-center">
                    <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-50 mb-4">
                        <i class='bx bx-shield-quarter text-4xl text-gray-300 font-thin'></i>
                    </div>
