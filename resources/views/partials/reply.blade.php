<div class="mt-3 border-l-2 border-gray-100 pl-2 md:pl-4" id="reply-{{ $reply->id }}">
    ...
    <div class="flex justify-between items-start mb-1">
        <div class="flex items-start">
            ...
            @if($reply->user->avatar)
                <img src="{{ asset('storage/' . $reply->user->avatar) }}" class="w-6 h-6 rounded-full object-cover border border-gray-200 mr-2 flex-shrink-0">
            @else
                <div class="w-6 h-6 rounded-full bg-gray-100 border border-gray-200 flex-shrink-0 flex items-center justify-center text-gray-500 text-[10px] font-bold mr-2">
            ...
            <div class="flex flex-wrap items-baseline gap-1">
                <a href="{{ route('user.profile', $reply->user->id) }}" class="text-xs font-bold text-gray-700 hover:text-maroon-700 leading-tight">
                    {{ $reply->user->name }}
                </a>

                {{-- REPLY AUTHOR FLAIR (Fixed to Acronyms) --}}
                @if($reply->user->member_type === 'student' && $reply->user->course)
                    <span class="text-[9px] font-bold text-blue-600 bg-blue-50 px-1 py-0.5 rounded border border-blue-100 leading-none" title="{{ $reply->user->course->name }}">
                        {{ $reply->user->course->acronym }}
                    </span>
                @elseif($reply->user->member_type === 'teacher' && $reply->user->departmentInfo)
                    <span class="text-[9px] font-bold text-purple-600 bg-purple-50 px-1 py-0.5 rounded border border-purple-100 leading-none" title="{{ $reply->user->departmentInfo->name }}">
                        {{ $reply->user->departmentInfo->acronym ?? 'Teacher' }}
                    </span>
                @endif

                {{-- Date (Shortened for Mobile: "2h", "1d") --}}
                <span class="text-[10px] text-gray-400 whitespace-nowrap ml-0.5">{{ $reply->created_at->diffForHumans(null, true, true) }}</span>
                
                @if($reply->updated_at->diffInMinutes($reply->created_at) > 5)
                    <span class="text-[9px] text-gray-300 italic" title="Edited {{ $reply->updated_at->diffForHumans() }}">(ed.)</span>
                @endif

                {{-- PENDING REVIEW BADGE --}}
                @if($reply->status === 'pending_review')
                    <span class="text-[9px] font-bold text-red-600 bg-red-50 px-1.5 py-0.5 rounded border border-red-100 animate-pulse">
                        <i class='bx bx-lock-alt align-middle'></i> Review
                    </span>
                @endif
            </div>
        </div>
    ...
    {{-- CONTENT --}}
    <div class="prose prose-sm prose-stone mb-1 text-xs md:text-sm text-gray-600 leading-snug">
        {!! $reply->content !!}
    </div>
    ...
    @auth
        @if(!$question->best_answer_id)
            <button onclick="toggleReplyForm('reply-form-{{ $reply->id }}')" class="text-[10px] md:text-xs text-gray-400 hover:text-maroon-700 font-medium flex items-center mb-2">
